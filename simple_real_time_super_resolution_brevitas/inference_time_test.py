...
def inference(cfg, model, dataloader, device):
    ...
    for idx, data in pbar:
        ...
        for b in range(_pred.shape[0]):
            pred = _pred[b]
            gt = hr_patch[b]
            print(f"lr: {lr_patch.shape}, pred: {_pred.shape}, gt: {gt.shape}")
            break
            ...
            #psnr = psnr_calc(_pred.detach().cpu().numpy().astype(np.uint8), gt.detach().cpu().numpy().astype(np.uint8))
...
if __name__ == '__main__':
    ...
    if args.weight is not None:
        ...
        if 'model_state_dict' in ckpt:
           ...
           model.load_state_dict(ckpt,strict = False)  # direct state_dict
    else: 
