...
from .plainRepConv import (
    PlainRepConv, PlainRepConv_st01, PlainRepConv_BlockV2,
    PlainRepConv_All, PlainRepConvClip, PlainRepConv_deploy,
    PlainRepConv_BlockV2_deploy
)
...
# 🔽 Add this line
from .QuantplainRepConv import QuantPlainRepConv

def get_model(cfg, device, mode='Train'):
    if cfg.model == 'plainsr':
        model = PlainSR(
            module_nums=cfg.m_plainsr,
            channel_nums=cfg.c_plainsr,
    ...
    elif cfg.model == 'QuantPlainRepConv':  # 🔽 Added case
        ...
        if mode == 'Train':
            ...
            model = PlainRepConv_BlockV2_deploy(
                module_nums=cfg.m_plainsr,
                channel_nums=cfg.c_plainsr,
                act_type=cfg.act_type,
                scale=cfg.scale,
        ...
        model = IMDN(
            in_nc=3, out_nc=3, nc=64, nb=8,
            upscale=cfg.scale, act_mode='L',
            upsample_mode='pixelshuffle', negative_slope=0.05
        )
